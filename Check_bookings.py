...
Button(root,text='Check Booking',bg='SpringGreen2',font='calibri 14 bold',command=checkbooking).grid(row=3,column=3)

def home():
    root.destroy()
    import Home
home_img=PhotoImage(file='.\\home.png')
Button(root,image=home_img,bg='PaleGreen2',command=home).grid(row=10,column=1,columnspan=4,pady=h//70)
