...
def buses():
    ...
    if to=='':
        messagebox.showerror('Error','Please enter the destination station')
    elif From=='':
        messagebox.showerror('Error','Please enter the source station')
    elif jdate=='':
        messagebox.showerror('Error','Please enter journey date')
    else:
...
def details():
    ...
    if booked_bus_id=='None':
        ...
        frame.grid(row=19,column=4,columnspan=7)
        Label(frame,text="Enter Login Details",font='calibri 18 bold').grid(row=19,column=4,columnspan=7)
        Label(frame,text="Username/Email:",font='calibri 14').grid(row=20,column=4)
        u_name=Entry(frame,width='20')
        u_name.grid(row=20,column=5,padx=w//60)
        Label(frame,text="Password:",font='calibri 14').grid(row=21,column=4,pady=h//90)
        u_pass=Entry(frame,width='20',show='*')
        u_pass.grid(row=21,column=5,padx=w//60)
        ...
        def login():
            ...
            cur.execute('''select count(*) from user_details where (username=? or email=?) and password=?''',(uname,uname,password))
            res=cur.fetchone()
            count=res[0]
            if count==0:
                messagebox.showerror('Error','Incorrect Username or password')
            else:
                Label(root,text='Fill Passenger details to book the bus ticket',bg='light blue',fg='red',font='calibri 24 bold').grid(row=24,column=2,columnspan=7,pady=h//30)
                
                Label(root,text='Name',font='calibri 14 bold').grid(row=25,column=1)
                e_name=Entry(root)
                e_name.grid(row=25,column=2)
                
                Label(root,text='Gender',font='calibri 14 bold').grid(row=25,column=3)
                e_gender=StringVar()
                option=('Male','Female','Other')
                e_gender.set('')
                g_menu=OptionMenu(root,e_gender,*option)
                g_menu.grid(row=25,column=4)
                
                Label(root,text='No. of seats',font='calibri 14 bold').grid(row=25,column=5)
                e_nos=Entry(root,width='3')
                e_nos.grid(row=25,column=6)
                
                Label(root,text='Mobile No',font='calibri 14 bold').grid(row=25,column=7)
                e_mob=Entry(root)
                e_mob.grid(row=25,column=8)
                
                Label(root,text='Age',font='calibri 14 bold').grid(row=25,column=9)
                e_age=Entry(root,width='3')
                e_age.grid(row=25,column=10)
                ...
                def confirm():
                    ...
                    cur.execute('''select seat_available from running_details where RBID=?''',(booked_bus_id,))
                    seat=cur.fetchall()
                    name=e_name.get()
                    age=e_age.get()
                    nos=e_nos.get()
                    ...
                    elif seat[0][0]<int(nos):
                        messagebox.showerror('Error','Number of seats exceeds available seat')
                    elif mob=='':
                ...
                Button(root,text='Book Seat',font='calibri 15 bold',bg='spring green',command=confirm).grid(row=25,column=11,padx=w//60)
                
        Button(frame,text="Login",font='calibri 13 bold',bg='spring green',command=login).grid(row=22,column=4)
        Label(frame,text='or').grid(row=22,column=5)

        def sign_up():
            root.destroy()
            import signup
        Button(frame,text="Sign Up",font='calibri 13 bold',bg='spring green',command=sign_up).grid(row=22,column=6,padx=w//70)
